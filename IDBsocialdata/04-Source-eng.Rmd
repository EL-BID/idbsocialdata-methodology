# Data source {#source-eng}

This section describes the data source used to calculate the indicators available in the [**Data and Indicators from Latin America and the Caribbean**](https://www.iadb.org/en/sharing-knowledge/data/social-data) platform and the IDBsocialdata packages available in [**R**](https://github.com/EL-BID/idbsocialdataR), [**Python**](https://github.com/EL-BID/idbsocialdatapy) y [**Stata**](https://github.com/EL-BID/idbsocialdata).

## IDB Harmonized Household Surveys {#HHS-e}

Household Surveys (HS) are used as a reference source to observe and analyze countries' demographic, economic, and social characteristics. The original databases are standardized following a common approach and structure to provide comparable data over time and between different nations, including uniform names, definitions, and disaggregation. This harmonization process allows users to save time in data processing and generate analytical information at the regional and subregional levels using uniform criteria. In addition, it facilitates the conduct of diagnostics, the formulation of strategies, and the research of specific social issues in each country in the region.

The Stata scripts and the documentation necessary to generate the data are available in the [***Git repository***](https://github.com/EL-BID/armonizacion_microdatos_encuestas_hogares_scl) to standardize the variables according to the established guidelines.

It is important to note that the primary source of the original HS microdata is the statistical institutes (INEs) of the countries in the region. In particular cases, the IDB co-produces some of these surveys. The following are the available sources and surveys:

| **Country** | **Source**                                                    | **Source**                                                      | **Year available**                    |
|------------------|-------------------|------------------|------------------|
...
\*The Mexico and Chile surveys are conducted every two and three years.

## IDB Harmonized Population and Housing Censuses {#HC-eng}

The Population and Housing Censuses aim to provide information on the population's number, structure, and geographical distribution in each country. As in the HD collection, it seeks to generate standardized and comparable data across countries and over time from information shared by the University of Minnesota's Integrated Public Use Microdata (IPUMS). In cases where censuses are unavailable in IPUMS, the NSOs of each country are used as a data source.

Based on the information obtained in [@IPUMS], standardization follows the guidelines established by the IDB. The Stata Scripts, and variable harmonization documents are available in the [***census harmonization repository***](). The source and available data are described below:

| **Country** | **Source**                                                   | **Year available**                 |
|------------------|----------------------------------|--------------------|
